...
module.exports = {
  async execute(interaction) {
    ...
    const buttonRow = new ActionRowBuilder()
      .addComponents(panelMessageButton, ticketChannelButton, ticketOpeningCategoryButton, ticketOpeningMessageButton, autoSaveTranscriptButton);
    const button2 = new ActionRowBuilder()
    .addComponents(ticketTranscriptChannelButton);

    embed.setDescription(
      `I. Panel Message\nII. Ticket Channel\nIII. Ticket Opening Category\nIV. Ticket Opening Message\nV. Auto Save Transcript\nVI. Ticket Transcript Channel`
    );

    await interaction.editReply({ embeds: [embed], components: [buttonRow,button2] });
  },
};
